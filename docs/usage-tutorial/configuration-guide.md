...
主题更新后，如果 `/node_modules/hexo-theme-redefine/_config.yml` 文件里出现了新的配置项，那么你必须从该文件中将它们复制到 `_config.redefine.yml` 中，并设置它们的值为你想要的选项。

关于文件比对，详见[文件比对](/usage-tutorial/advanced.html#%E6%96%87%E4%BB%B6%E6%AF%94%E5%AF%B9)
...
1. 在 Hexo 配置文件 `_config.redefine.yml` 最底下增加如下配置项。
   ...
   feed:
     ...
     limit: 20
   ```
1. 在 Keep 主题配置文件 `_config.redefine.yml` 开启 RSS。   
   ```yml
   rss:
     enable: true
   ```
